...
{* ABATECS *}
                        ...
                        {if $FIELD_MODEL->getFieldDataType() eq 'phone'}{*$USER_MODEL->get('phone_crm_extension')*}
                        {*$FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue'))*}

                        <a href="http://192.168.10.94/crmpbx/crm_c2c.php?phoneno={$FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue'))}&exten={$USER_MODEL->get('phone_crm_extension')}&type=summary&record=2"> {include file=$FIELD_MODEL->getUITypeModel()->getDetailViewTemplateName()|@vtemplate_path FIELD_MODEL=$FIELD_MODEL USER_MODEL=$USER_MODEL MODULE=$MODULE_NAME RECORD=$RECORD}</a>
